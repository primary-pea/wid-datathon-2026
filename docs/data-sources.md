...
| Global Fortification Data Exchange — https://fortificationdata.org | mandatory fortification status and legislation years by food vehicle | dataset snapshot, September 2026 | `feat_gd_any_fortification`, wheat-mandate year in [`derived/events.csv`](../derived/events.csv), fortification status in the residual ranking |
...
| Team files | the 49-country list; the cluster rules | — | [`scaffold/config.py`](../scaffold/config.py); [`scaffold/inputs/gifna_cluster_rules.csv`](../scaffold/inputs/gifna_cluster_rules.csv) |
